...
import { materialApi } from '../../../../services/api.service';
import { toast } from 'react-hot-toast';
import type { Material } from '../../../../types';

/**
 * Materials admin — same shape as the Categories screen (name, slug,
 ...
 * fields, and `image` is a plain URL string — the Material model has no file
...
export default function MaterialsPage() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (editItem) {
          await materialApi.update(editItem.id, payload);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this material? Products using it keep their record but lose the link.')) return;
    try {
  ...
  const handleActiveToggle = async (m: Material) => {
    const next = !m.isActive;
    setMaterials(prev => prev.map(x => x.id === m.id ? { ...x, isActive: next } : x));
    try {
      await materialApi.update(m.id, { isActive: next });
    } catch {
      setMaterials(prev => prev.map(x => x.id === m.id ? { ...x, isActive: !next } : x));
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3, gap: 2, flexWrap: 'wrap' }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            The wood/material taxonomy shoppers filter by and products are tagged with.
          </Typography>
        </Box>
      ...
      <TextField size="small" placeholder="Search materials…" sx={{ width: 260, mb: 2.5 }}
      ...
      {loading ? (
        ...
        <Stack spacing={1.5}>
          ...
          {filtered.length === 0 && (
            <Typography color="text.secondary" sx={{ p: 4, textAlign: 'center' }}>No materials found</Typography>
          )}
        </Stack>
      )}
      ...
      <Dialog open={dialogOpen} onClose={closeDialog} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 700, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {editItem ? `Edit: ${editItem.name}` : 'Add Material'}
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <DialogActions sx={{ p: 2, gap: 1 }}>
            ...
            <Button type="submit" variant="contained" disabled={formik.isSubmitting}
              ...
              {formik.isSubmitting ? 'Saving…' : editItem ? 'Save Changes' : 'Create Material'}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
