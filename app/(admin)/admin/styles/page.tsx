...
import { styleApi } from '../../../../services/api.service';
import { toast } from 'react-hot-toast';
import type { Style } from '../../../../types';

/**
 * Styles admin — same shape as the Categories screen (name, slug,
 ...
 * fields, and `image` is a plain URL string — the Style model has no file
...
export default function StylesPage() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (editItem) {
          await styleApi.update(editItem.id, payload);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this style? Products using it keep their record but lose the link.')) return;
    try {
  ...
  const handleActiveToggle = async (m: Style) => {
    const next = !m.isActive;
    setStyles(prev => prev.map(x => x.id === m.id ? { ...x, isActive: next } : x));
    try {
      await styleApi.update(m.id, { isActive: next });
    } catch {
      setStyles(prev => prev.map(x => x.id === m.id ? { ...x, isActive: !next } : x));
  ...
  const filtered = styles.filter(m => !search || m.name.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3, gap: 2, flexWrap: 'wrap' }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            The design-style taxonomy shoppers filter by and products are tagged with (e.g. Scandinavian, Industrial, Boho).
          </Typography>
        </Box>
      ...
      <TextField size="small" placeholder="Search styles…" sx={{ width: 260, mb: 2.5 }}
      ...
      {loading ? (
        ...
        <Stack spacing={1.5}>
          ...
          {filtered.length === 0 && (
            <Typography color="text.secondary" sx={{ p: 4, textAlign: 'center' }}>No styles found</Typography>
          )}
        </Stack>
      )}
      ...
      <Dialog open={dialogOpen} onClose={closeDialog} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 700, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {editItem ? `Edit: ${editItem.name}` : 'Add Style'}
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <DialogActions sx={{ p: 2, gap: 1 }}>
            ...
            <Button type="submit" variant="contained" disabled={formik.isSubmitting}
              ...
              {formik.isSubmitting ? 'Saving…' : editItem ? 'Save Changes' : 'Create Style'}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
